...
window.addEventListener("load", function () {
  const colors = {
   ...
   function myCall ( color ){
    let vElement;
    //create element
    vElement = document.createElement("h1");  
    vElement.innerText = "I know how binding works in JS";
    document.querySelector("body").appendChild(vElement);
    //
   ...
   function myApply (title){
    this.title =  "Заголовок 1";
    document.title = this.title;
   };

   myCall.call( colors, "red" );
   myApply.apply( colors, ["Заголовок"] );

   setTimeout(() =>  { myBind.bind( colors )() }, 3000);
